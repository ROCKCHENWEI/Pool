...
- `Conformance package catalogs`：`GET /api/provider-conformance-packages`、`GET /api/software-conformance-packages`、`GET /api/agent-conformance-packages`、`GET /api/integration-conformance-packages` 与对应 `pool-cli *-conformance-packages` / MCP read tools 会从 asset ledger 和本地 manifest 恢复已生成验收包目录，供 Hermes/Agent 断线后继续接手。
...
- `Core architecture readiness gate`：`GET /api/core-architecture-readiness`、`pool://core-architecture-readiness`、`pool-cli core-architecture-readiness` 和 MCP tool `pool_core_architecture_readiness` 会单独证明本地核心运行架构是否闭合，覆盖 project/workflow、节点执行计划、Hermes/Agent、Provider/软件合同、Unreal 优先、本地 indexed assets、三类输出和 handoff/MCP；`GET /api/core-architecture-gate`、`pool://core-architecture-gate`、`pool-cli core-architecture-gate --require-ready` 和 MCP tool `pool_core_architecture_gate` 提供可失败的硬门；真实 Provider、真实软件和外部视觉模型证据仍由严格 `prd-completion-gate` 阻断。
- `Core architecture local smoke`：`cargo run -q -p pool-core --example run_prd_readiness_smoke -- target/core-architecture-readiness-smoke` 会运行本地 ContentBurst、mock 3DGS、Unreal mock、三类输出结果、Provider/软件/桌面 trace 本地证据，并断言 `core_architecture_gate.ready_for_core_architecture=true`，随后写出核心架构证明包；默认不导入真实生产证据，所以 PRD completion gate 仍可保持 incomplete。
...
- `Web core architecture gate`：Web 连接 Runtime 后会读取 `/api/core-architecture-readiness` 和 `/api/core-architecture-gate`，在 PRD 面板展示“核心架构门槛已满足/未满足”，并保留 PRD completion gate 对生产证据的独立阻断状态。
- `Web core architecture package`：PRD 面板会读取 `GET /api/core-architecture-packages` 恢复最近核心架构证明包，也可调用 `POST /api/core-architecture-package`，把核心架构 readiness、硬门 gate、runtime graph、execution plan、handoff、三类输出 catalog、严格 PRD completion gate 和可选 snapshot 写入本地 `control/core-architecture/`，用于在真实生产证据收口前归档“运行框架已闭环”的机器证明。
...
- `Runtime handoff package control`：Web prototype 的 Runtime Handoff 卡片可调用 `POST /api/handoff-packages`，把接管 runbook、preflight、runtime graph、integration readiness、worker self-check runner、worker preflight、带 operator checklist 的 manifest、5 人团队分工和 snapshot 写入 `output/control/handoff/`；连接 Runtime 时会读取 `GET /api/handoff-packages` 的只读 catalog，重新发现最近接管包，并在面板中显示 manifest、operator checklist、Agent/MCP 入口、readiness 和 worker smoke 路径。
...
- `Pool core architecture readiness resource`：`pool://core-architecture-readiness` 使用同一份 snapshot 输出 `architecture_gate`，用于区分“本地核心架构已完成”和“真实生产证据尚未收口”。
- `Pool core architecture package control`：`GET /api/core-architecture-gate`、`pool://core-architecture-gate`、`pool-cli core-architecture-gate --require-ready` 和 MCP tool `pool_core_architecture_gate` 会对本地核心架构闭环执行硬性判定；`GET /api/core-architecture-packages`、`pool://core-architecture-packages`、`pool-cli core-architecture-packages` 和 MCP tool `pool_core_architecture_packages` 会从资产账本恢复已生成核心架构证明包；`POST /api/core-architecture-package`、`pool-cli core-architecture-package` 和 MCP tool `pool_core_architecture_package` 会写出包含 `2-core-architecture-gate.json` 的本地核心架构证明包，同步入 task、assets 和 events；它不替代 `prd-completion-gate`，只证明当前 SQLite runtime、节点执行计划、handoff、输出 catalog 和 completion gate 状态可被 Agent/Hermes 复核。
...
- `RuntimeHttpServer PRD completion packages`：`GET /api/prd-completion-packages` 会从 SQLite asset ledger 与本地 manifest 恢复已生成 PRD 完成证明包，和 `pool://prd-completion-packages`、`pool-cli prd-completion-packages`、MCP tool `pool_prd_completion_packages` 共用 payload；`POST /api/prd-completion-package` 继续负责物化当前 readiness/gate/production evidence requirements。
- `RuntimeHttpServer core architecture readiness/package`：`GET /api/core-architecture-readiness` 暴露同源只读核心架构完成门槛，和 `pool://core-architecture-readiness`、`pool-cli core-architecture-readiness`、`pool_core_architecture_readiness` 共用 payload；`GET /api/core-architecture-gate?require_ready=true` 会在未满足本地核心架构门槛时返回 HTTP 428；`GET /api/core-architecture-packages` 会从 SQLite asset ledger 和本地 manifest 恢复已生成核心架构证明包；`POST /api/core-architecture-package` 会物化同一组核心架构证据文件，供 `pool-cli core-architecture-package` 和 MCP tool `pool_core_architecture_package` 调用。
...
- `RuntimeHttpServer handoff package dispatch`：`POST /api/handoff-packages` 已能触发 `RuntimeHandoffPackageRunner`，生成 `1-runtime-handoff.json`、`2-runtime-preflight.json`、`3-runtime-graph.json`、`7-integration-readiness.json`、`8-runtime-handoff-package-manifest.json`、`5-worker-self-checks.sh`、`6-worker-self-checks-preflight.json` 和可选 `4-runtime-snapshot.json`，并在 report 直接返回 `operator_checklist`、`agent_entrypoint` 和 `mcp_resources`，同时返回 task 和刷新后的 snapshot；`GET /api/handoff-packages`、`pool://runtime-handoff-packages`、`pool-cli runtime-handoff-packages` 和 MCP tool `pool_runtime_handoff_packages` 会从 SQLite asset ledger 与本地 manifest 恢复已生成包的 ready/missing 状态、operator checklist 和 Agent/MCP 入口。
