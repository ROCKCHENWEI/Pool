...
impl SeedanceAdapter {
    pub fn new(api_key: String) -> Self {
        Self {
            api_key,
            client: Client::new(),
            base_url: "https://api.seedance.ai/v1".to_string(),
        }
    }
}

#[async_trait]
impl VideoGeneratorAdapter for SeedanceAdapter {
    fn name(&self) -> &str { "seedance" }

    async fn text_to_video(&self, _prompt: &str, _negative_prompt: Option<&str>, _config: VideoGenerationConfig) -> Result<GenerationTask> {
        Ok(GenerationTask { task_id: format!("seedance_{}", uuid::Uuid::new_v4()), status: TaskStatus::Pending, progress: 0.0 })
    }
