...
pool-cli core-architecture-readiness
...
pool-cli mcp pool://core-architecture-readiness
...
pool-cli prd-completion-packages
...
`core-architecture-readiness` 会调用 `/api/core-architecture-readiness`，读取同源 `pool://core-architecture-readiness` 核心架构完成门槛。它只证明本地运行框架闭环，覆盖 project/workflow、节点执行计划、Hermes/Agent、Provider/软件合同、Unreal 优先、本地 indexed assets、三类输出和 handoff/MCP；真实 Provider、真实软件和外部视觉模型证据仍由 `prd-completion-gate --require-complete` 严格阻断。MCP tool `pool_core_architecture_readiness` 返回同一份只读 payload。

`core-architecture-gate --require-ready` 会调用 `/api/core-architecture-gate?require_ready=true`，只返回核心架构硬门 wrapper。未满足本地核心架构门槛时会返回 `core_architecture_gate_incomplete` / HTTP 428，适合 Agent/Hermes/CI 在生产证据还未收口前先阻断“核心架构已完成”的误报。MCP resource `pool://core-architecture-gate` 与 MCP tool `pool_core_architecture_gate` 返回同一语义。

`core-architecture-packages` 会调用 `GET /api/core-architecture-packages`，从 SQLite asset ledger 和本地 `control/core-architecture/8-core-architecture-package-manifest.json` 恢复已生成核心架构证明包 catalog。它会返回 manifest、readiness、core architecture gate、runtime graph、execution plan、handoff、output packages、strict PRD gate、snapshot 路径、ready 状态、命令和缺失本地文件列表；MCP resource `pool://core-architecture-packages` 与 MCP tool `pool_core_architecture_packages` 返回同一份只读 catalog。

`core-architecture-package` 会调用 `POST /api/core-architecture-package`，把核心架构 readiness、runtime graph、execution plan、handoff、output packages、严格 PRD completion gate、manifest 和可选 snapshot 写到 `<output-dir>/control/core-architecture/`，并同步入 task、assets 和 events。常用命令是 `pool-cli --project demo core-architecture-package --output-dir worlds/demo/output --include-snapshot`；MCP tool `pool_core_architecture_package` 暴露同一入口给 Hermes/Agent。它用于归档“本地核心架构已闭环”的证据，不会绕过真实生产证据的 `prd-completion-gate`。

`prd-completion-packages` 会调用 `GET /api/prd-completion-packages`，从 SQLite asset ledger 和本地 `control/prd-completion/4-prd-completion-package-manifest.json` 恢复已生成 PRD 完成证明包 catalog。它会返回 readiness、completion gate、production evidence requirements、manifest、snapshot 路径、ready 状态、operator checklist、命令和缺失本地文件列表；MCP resource `pool://prd-completion-packages` 与 MCP tool `pool_prd_completion_packages` 返回同一份只读 catalog。
...
`provider-conformance-packages`、`software-conformance-packages`、`agent-conformance-packages` 和 `integration-conformance-packages` 是只读目录命令，分别调用对应 `GET /api/*-conformance-packages`。它们从 SQLite asset ledger 和本地 manifest 恢复已生成包，返回 ready 状态、manifest/runner/preflight/contract 路径、命令、缺失文件和本地优先策略；MCP tools `pool_provider_conformance_packages`、`pool_software_conformance_packages`、`pool_agent_conformance_packages`、`pool_integration_conformance_packages` 暴露同一目录给 Hermes/Agent。
...
`production-evidence-handoff-packages` 会读取 `GET /api/production-evidence/handoff-packages`，从 assets ledger 和本地 manifest/runner preflight 恢复已经写出的生产证据交付包 catalog。响应会列出 package ready 状态、manifest/run-plan/bundle/runner 路径、item 文件数、Provider gateway worker 启动命令和 software bridge worker 启动命令；同一 payload 也暴露为 `pool://production-evidence-handoff-packages` 和 MCP tool `pool_production_evidence_handoff_packages`，方便 Hermes/Agent 在重连后继续执行最近交付包。
...
`runtime-handoff-packages` 会调用 `GET /api/handoff-packages`，从 SQLite asset ledger 和本地 handoff manifest 恢复已生成接管包 catalog。它只读不写 SQLite，适合 Agent/Hermes 在重新连接 runtime 后找回最近的接管包、operator checklist、`agent_entrypoint.mcp_stdio` 和 `pool://...` 资源列表。
...
cargo run -p pool-cli -- --db target/pool-cli-smoke/pool-runtime.sqlite --project demo core-architecture-readiness
...
cargo run -p pool-cli -- --db target/pool-cli-smoke/pool-runtime.sqlite --project demo mcp pool://core-architecture-readiness
...
生成后可用只读 catalog 找回最近接管包：

```bash
cargo run -p pool-cli -- --db target/pool-cli-smoke/pool-runtime.sqlite --project demo runtime-handoff-packages
```
